...
source "${ukb_utils_dir}/qsub_utils.sh"
...
get_hail_memory_slurm() {
  ...
  if [[ "${SLURM_JOB_PARTITION}" = "short" || "${SLURM_JOB_PARTITION}" = "long" || "${SLURM_JOB_PARTITION}" = "epyc" ]]; then
    local _mem_per_slot=15
  else
...
set_up_hail() {
  ...
  # without this "set" below we get the following errors
  # line 65: CONDA_BACKUP_CXX: unbound variable
  set +u
  set_up_conda
  set -u
  ...
  if [ ! -z ${_mem} ]; then
    ...
    >&2 echo "Warning: Hail memory could not be set for cluster '${_cluster}'"
  fi
  echo "Running Hail (v${hversion}) with ${_mem}gb of memory on ${_cluster}."
}

set_up_hail_debug() {

  #local hversion="undef"
  local hversion="${1:-0.2.105}"
  if [ -z "${spark_dir-}" ]; then
    spark_dir='/well/lindgren/flassen/spark/default'
    echo "Note: spark_dir has not been set. Defaulting to ${spark_dir}."
  fi
...
set_up_pythonpath_legacy() {
  ...
  export PYTHONPATH="${PYTHONPATH}:/well/lindgren/flassen/ressources/ukb/ukb_utils/python/ukb_utils"
  export PYTHONPATH="${PYTHONPATH}:/well/lindgren/UKBIOBANK/flassen/projects/KO/wes_ko_ukbb/utils/modules/python"
}
...
set_up_vep105() {
  ...
  export PERL5LIB="/apps/eb/skylake/software/EnsEMBLCoreAPI/96.0-r20190601-foss-2019a-Perl-5.28.1/lib/perl5/site_perl/5.28
.1/x86_64-linux-thread-multi:$PERL5LIB"
}
